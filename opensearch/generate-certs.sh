...
OPENDISTRO_DN="/C=MA/ST=1337/L=KENITRA/O=EXAMPLE"   # Edit here and in opensearch.yml
...
do
    ...
    openssl x509 -req -extfile <(printf "subjectAltName=DNS:localhost,IP:172.0.0.1,DNS:$NODE_NAME") -in "certs/$NODE_NAME/$NODE_NAME.csr" -CA certs/ca/ca.pem -CAkey certs/ca/ca.key -CAcreateserial -sha256 -out "certs/$NODE_NAME/$NODE_NAME.pem"
